...
impl AuthKey {
    ...
    pub fn load_or_create(path: &Path) -> Result<Self> {
        ...
        if rotated_path.exists() {
            ...
            info!("Loaded existing authentication keys with rotation");
            Ok(key)
        } else if path.exists() {
            ...
            info!("Loaded and upgraded legacy authentication key to rotation format");
            Ok(key)
        } else {
            // Create a new key and save it
            let key = Self::new_random()?;
            key.save_to_file(path)?;
            info!("Generated and saved new authentication key with rotation support");
            Ok(key)
        }
    }
    ...
    pub fn generate_tag<T: Serialize>(&self, message: &T) -> Result<String> {
        ...
        // Remove timestamp from HMAC calculation to make it independent of time differences
    ...
    pub fn verify_tag<T: Serialize>(&self, message: &T, tag: &str) -> Result<bool> {
        ...
        match parts.len() {
            ...
            2 => {
                ...
                info!(
                    "Current key timestamp: {}, {} previous keys available",
                    self.current_key.created_at,
                ...
                debug!(
                    "Trying to verify with current key (timestamp: {})",
                    self.current_key.created_at
                );
                if self.verify_with_key(message, tag_value, &self.current_key.key, 0)? {
                    info!("Successfully verified with current key");
                    return Ok(true);
                }
                ...
                for (i, prev_key) in self.previous_keys.iter().enumerate() {
                    debug!(
                        "Trying to verify with previous key {} (timestamp: {})",
                        i, prev_key.created_at
                    );
                    if self.verify_with_key(message, tag_value, &prev_key.key, 0)? {
                        info!("Successfully verified with previous key {}", i);
                        return Ok(true);
                    }
                }
