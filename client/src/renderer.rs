...
pub fn render_mini_map(state: &GameState, _player_pos: &Position) {
    ...
    println!(
        "{}",
        "Mini-map: (You are '@', others are 'O'):".cyan().bold()
    );

    // Debug info about world dimensions
    println!(
        "World boundaries: X: [{:.1}, {:.1}], Y: [{:.1}, {:.1}]",
    ...
    for (id, player) in &state.players {
        ...
        println!(
            "Player {}: World: ({:.1}, {:.1}) → Map: ({}, {}){}",
            player.name,
            player.position.x,
            player.position.y,
            map_x,
            map_y,
            if Some(id) == current_player_id.as_ref() {
                " (YOU)"
            } else {
                ""
            }
    ...
    println!(
        "  Each {} command moves exactly one cell",
        "'move <direction>'".green().bold()
    );
    println!(
        "  Valid directions: {}, {}, {}, {}",
...
pub fn render_game_state(state: &GameState) {
    ...
    match &state.player_id {
        Some(id) => {
            ...
            println!(
                "Players in game: {}",
                state.players.len().to_string().yellow().bold()
            );

            for (id, player) in &state.players {
                if Some(id) != state.player_id.as_ref() {
                    let distance_x = if let Some(current_player) = state
                        .player_id
                        .as_ref()
                        .and_then(|pid| state.players.get(pid))
                    {
                        ((player.position.x - current_player.position.x).powi(2)
                            + (player.position.y - current_player.position.y).powi(2))
                        .sqrt()
                    } else {
                        f32::MAX
                    };
    ...
    if state.player_id.is_some() {
        ...
        println!(
            "    (use cardinal: n,s,e,w or diagonal: ne,nw,se,sw or arrows: up,down,left,right)"
        );
